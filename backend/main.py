...
app = FastAPI(title="Nesting API", description="API para optimización de nesting de piezas irregulares")

# Configurar CORS para permitir conexiones desde React
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:3000", "http://localhost:3001"],  # Puertos comunes de React
...
# Instancia global del motor de nesting
nesting_engine = None
    ...
    try:
        ...
        for piece_data in request.pieces:
            for _ in range(piece_data.quantity):
                polygon = nesting_engine.points_to_polygon(piece_data.points)
                polygon = nesting_engine.normalize_polygon(polygon)
                polygons.append(polygon)
                piece_ids.append(piece_data.id)
        
        # Ejecutar algoritmo seleccionado
        if request.algorithm == "genetic":
            positions = nesting_engine.genetic_algorithm(polygons, generations=30)
        elif request.algorithm == "bottom_left":
            positions = nesting_engine.bottom_left_fit(polygons)
        ...
        for i, (polygon, (x, y, rotation)) in enumerate(zip(polygons, positions)):
            ...
            transformed = nesting_engine.rotate_polygon(polygon, rotation)
            transformed = nesting_engine.normalize_polygon(transformed)
            transformed = translate(transformed, x, y)
            
            # Convertir a formato de respuesta
            points = nesting_engine.polygon_to_points(transformed)
