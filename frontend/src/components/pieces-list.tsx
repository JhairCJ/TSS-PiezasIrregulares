...
import { Trash2, Edit3, Save, X, Pencil } from "lucide-react"
...
const PiecesList: React.FC<PiecesListProps> = ({ pieces, onUpdatePieces, onClearAll, onEditShape }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-medium">Lista de Piezas</h3>
        <button
          onClick={onClearAll}
      ...
      <div className="mt-4 p-3 bg-blue-50 rounded-lg">
        <div className="text-sm text-blue-800">
          <strong>Total:</strong> {pieces.length} tipos de figuras,{" "}
          {pieces.reduce((sum, piece) => sum + piece.quantity, 0)} piezas en total
        </div>
      </div>
    </div>
  )
